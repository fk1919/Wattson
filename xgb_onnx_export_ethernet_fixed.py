...
pipe = Pipeline([('scaler', scaler), ('xgb', xgb_clf)])
...
update_registered_converter(
    type(xgb_clf), 'XGBoostXGBClassifier',
...
onnx_model = convert_sklearn(
    ...
    options={id(xgb_clf): {'zipmap': False}},
)
print(f"Conversion time: {time.time()-t0:.1f}s")

with open("xgb_pipeline_ethernet_fixed.onnx", "wb") as f:
    f.write(onnx_model.SerializeToString())

size_mb = os.path.getsize("xgb_pipeline_ethernet_fixed.onnx") / 1e6
print(f"Saved: xgb_pipeline_ethernet_fixed.onnx ({size_mb:.1f} MB)")

import onnxruntime as ort
sess = ort.InferenceSession("xgb_pipeline_ethernet_fixed.onnx", providers=['CPUExecutionProvider'])
...
# Validation - with the session-based test set (feature_matrix_ethernet_fixed.npz + sessions)
